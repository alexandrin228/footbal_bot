...
def format_performance_report(report: dict) -> str:
    """Raport periodic de performanță (Brier Score, hit rate, calibrare) din Prediction Ledger."""
    n = report.get("n", 0)
    if n == 0:
        return (
            "📊 <b>RAPORT PERFORMANȚĂ BOT</b>\n\n"
            "Niciun meci rezolvat încă - revin cu date de îndată ce se termină "
            "primele meciuri urmărite."
        )

    lines = [
        "📊 <b>RAPORT PERFORMANȚĂ BOT</b>",
        f"<i>Bazat pe {n} predicții rezolvate</i>",
        "───────────────────",
        f"Brier Score (1X2): <b>{report['brier_score']}</b>",
        "<i>(mai mic = mai bine; 0 = perfect, ~0.63 = ghicit la întâmplare)</i>",
    ]
    if report.get("over_2_5_hit_rate") is not None:
        lines.append(f"Hit rate Over/Under 2.5: <b>{report['over_2_5_hit_rate']}%</b>")
    if report.get("btts_hit_rate") is not None:
        lines.append(f"Hit rate BTTS: <b>{report['btts_hit_rate']}%</b>")
    if report.get("value_bet_hit_rate") is not None:
        lines.append(
            f"Hit rate value bets: <b>{report['value_bet_hit_rate']}%</b> "
            f"({report['value_bet_count']} urmărite)"
        )

    if report.get("calibration"):
        lines.append("───────────────────")
        lines.append("<b>🎯 CALIBRARE</b> (prezis vs realitate)")
        lines.append("<code>")
        lines.append(f"{'Interval':<10}{'n':>4}  {'prezis':>7}  {'real':>7}")
        for c in report["calibration"]:
            note = "" if c["n"] >= 10 else "*"
            lines.append(f"{c['range']:<10}{c['n']:>4}  {c['predicted_avg']:>6}%  {c['actual_freq']:>6}%{note}")
        lines.append("</code>")
        lines.append("<i>Dacă prezis ≈ real, modelul e bine calibrat. * = sub 10 meciuri, orientativ.</i>")

    if n < 50:
        lines.append("")
        lines.append(
            f"⚠️ <i>Doar {n} predicții rezolvate - sub 50 de meciuri, aceste cifre "
            f"sunt orientative, nu concluzive statistic.</i>"
        )

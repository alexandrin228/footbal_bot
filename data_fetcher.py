...
def find_final_score(all_matches: list, home_name: str, away_name: str):
    """
    Caută scorul final al unui meci specific în lista de meciuri finalizate
    (football-data.org). Folosește potrivire flexibilă de nume, pentru că
    numele salvate în ledger vin de la The Odds API, format ușor diferit.
    Returnează (goluri_gazde, goluri_oaspeți) sau None dacă meciul încă nu
    apare ca finalizat în sursă.
    """
    h_lower, a_lower = home_name.lower(), away_name.lower()
    for m in all_matches:
        home = m["homeTeam"]["name"].lower()
        away = m["awayTeam"]["name"].lower()
        if (h_lower in home or home in h_lower) and (a_lower in away or away in a_lower):
